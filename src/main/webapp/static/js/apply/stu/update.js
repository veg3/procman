layui.config({
...
}).use(['form','jquery','table','layer','laydate','laytpl','autocomplete'], function() {
    let form = layui.form,
        $ = layui.jquery,
        table = layui.table,
    ...
    form.on("submit(btns)",function(data) {
        ...
        layer.confirm('确定提交吗?', {icon: 3, title:'提示'}, function(index){
            $.post('/item/update/'+saveToken+"/"+tid,data.field,function (info) {
    ...
    $(".adds").click(function () {     //增加项目成员 subs
        ...
        $(node).find("input").attr("class","layui-input")
                             .attr("autocomplete","off");
        var sno = $(node).find(":input[name$='stuId']");
    ...
    $(".addt").click(function () {
        ...
        var node = $("<tr><td>第二导师</td>" +
            ...
            "<td><input name='tumb[1].name'  placeholder='搜索选择导师' id='tupNt'></td>" +
        ...
        $(node).find("input").attr("class","layui-input")
                             .attr("autocomplete","off")
        addp(tb,node,num,text);
        var tupn = $(node).find("input[id='tupNt']");
        searchtup(tupn);
        return false;
    });
    $(".subt").click(function () {
    ...
    $(".tst :input[name$='stuId']").each(function () {
