layui.define(['jquery', 'laytpl', 'layer'], function (exports) {
    "use strict";
    layui.link('/static/js/modules/autocomplete.css');
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        layer = layui.layer,
        ...
        job.prototype.pullData = function () {
            ...
            (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
                ...
                success: function (resp) {
                    return 0 != eval('resp.' + _config.response.code) ? layer.msg(eval('resp.' + _config.response.msg)) : _config.data[_self.index] = eval('resp.' + _config.response.data), _self.renderData(_config.data[_self.index])
                },
                error: function () {
                    hint.error("请求失败")
                },
            ...
            _elem.on('focus', function () {
                ...
                $(document).on('click', function (e) {
                    ...
                    if (_e !== undefined) {
                        ...
                        _elem.val(laytpl(_config.template_val).render(_config.data[_self.index][_e.data("index")])), _config.onselect == undefined || _config.onselect(_config.data[_self.index][_e.index()])
                    }
    ...
    system.init(), exports(module, system);
})