...
layui.define(["element","jquery"],function(exports){
    ...
    Tab.prototype.navBar = function(strData){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].isSpread ==0 || data[i].isSpread == undefined){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                if(data[i].menuIcon != undefined && data[i].menuIcon != ''){
                    if(data[i].menuIcon.indexOf("icon-") != -1){
                        ulHtml += '<i class="seraph '+data[i].menuIcon+'" data-icon="'+data[i].menuIcon+'"></i>';
                    }else{
                        ulHtml += '<i class="layui-icon" data-icon="'+data[i].menuIcon+'">'+data[i].menuIcon+'</i>';
                    }
                }
                ulHtml += '<cite>'+data[i].meunName+'</cite>';
                ...
                for(var j=0;j<data[i].children.length;j++){
                    ...
                    if(data[i].children[j].menuIcon != undefined && data[i].children[j].menuIcon != ''){
                        if(data[i].children[j].menuIcon.indexOf("icon-") != -1){
                            ulHtml += '<i class="seraph '+data[i].children[j].menuIcon+'" data-icon="'+data[i].children[j].menuIcon+'"></i>';
                        }else{
                            ulHtml += '<i class="layui-icon" data-icon="'+data[i].children[j].menuIcon+'">'+data[i].children[j].menuIcon+'</i>';
                        }
                    }
                    ulHtml += '<cite>'+data[i].children[j].meunName+'</cite></a></dd>';
                }
                ulHtml += "</dl>";
            }else{
                ...
                ulHtml += '<cite>'+data[i].meunName+'</cite></a>';
            }
            ulHtml += '</li>';
        }
        return ulHtml;
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><span>&nbsp;&nbsp;<i class="layui-icon layui-icon-app"></i><cite>系统功能</cite></span></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
	...
	$(".closePageOther").on("click",function(){
		if($("#top_tabs li").length>2 && $("#top_tabs li.layui-this cite").text()!="系统功能"){
			var menu = JSON.parse(window.sessionStorage.getItem("menu"));
		...
		}else if($("#top_tabs li.layui-this cite").text()=="系统功能" && $("#top_tabs li").length>1){
			$("#top_tabs li").each(function(){
				if($(this).attr("lay-id") != '' && !$(this).hasClass("layui-this")){
					element.tabDelete("bodyTab",$(this).attr("lay-id")).init();
